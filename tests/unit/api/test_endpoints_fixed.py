...
Tests individual API endpoint functions,                 result = mock_service_instance.generate(
...
class TestImageGenerationAPI:
    ...
    def test_generate_image_endpoint_success(self):
        ...
        with patch('services.ai.image_generation.core.generation_engine.GenerationEngine') as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.generate.return_value = {
            ...
            def mock_generate_image(request):
                ...
                result = mock_service_instance.generate(
                    prompt=request['prompt'],
                    width=request.get('width', 512),
                    height=request.get('height', 512),
                    num_inference_steps=request.get('num_inference_steps', 20)
    ...
    def test_generate_image_endpoint_service_error(self):
        ...
        with patch('services.ai.image_generation.core.generation_engine.GenerationEngine') as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.generate.side_effect = Exception("GPU out of memory")
            
            def mock_generate_image(request):
                try:
                    result = mock_service_instance.generate(**request)
